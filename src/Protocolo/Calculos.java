...
public class Calculos {
    ...
    public float multa_res = 0;
    public float multa_com = 0;
    ...
    public int DiasParado(String vencto) {
        int retorno = 0;
        // Checa se é sabado ou domingo
        int sabdom = Dates.isSabadoOuDomingo(Dates.StringtoDate(vencto, "dd/MM/yyyy"));
        int difdata = Dates.DateDiff(Dates.DIA,  new Date(), Dates.StringtoDate(vencto, "dd/MM/yyyy")) + 1;
    ...
    public int Feriados(String vencto) {
        int retorno = 0;
        String pdia = vencto.substring(0, 2);
        String pmes = vencto.substring(3, 5);
        String[][] aferiado = null;
    ...
    public static float[] RetAluguel(String campos) {
        String[] campo = campos.split(";");
        int i = 0;
        float fAL = 0, fDC = 0, fDF = 0, fDc = 0, fDf = 0;
        for (i=0;i<=campo.length-1;i++) {
            String[] scampo = campo[i].split(":");
            
            if (scampo[4].equalsIgnoreCase("AL")) {
                fAL += LerValor.StringToFloat(LerValor.FormatNumber(scampo[2], 2));
            }
            
            if (scampo[4].equalsIgnoreCase("DC") && scampo[5].equalsIgnoreCase("AL")) {
                fDC += LerValor.StringToFloat(LerValor.FormatNumber(scampo[2], 2));
            } else if (scampo[4].equalsIgnoreCase("DC") && !scampo[5].equalsIgnoreCase("AL")){
                fDc += LerValor.StringToFloat(LerValor.FormatNumber(scampo[2], 2));
            }
            
            if (scampo[4].equalsIgnoreCase("DF") && scampo[5].equalsIgnoreCase("AL")) {
                int pos = FuncoesGlobais.IndexOf(scampo, "ET");
                if (pos > -1) {
                    fDF += LerValor.StringToFloat(LerValor.FormatNumber(scampo[2], 2));
                } 
            } else if (scampo[4].equalsIgnoreCase("DF") && !scampo[5].equalsIgnoreCase("AL")) {
                int pos = FuncoesGlobais.IndexOf(scampo, "ET");
                if (pos > -1) {
                    fDf += LerValor.StringToFloat(LerValor.FormatNumber(scampo[2], 2));
                }
        ...
        return new float[] {fAL, fDC, fDF, fDc, fDf};
    }
    ...
    public String TipoImovel() throws SQLException {
