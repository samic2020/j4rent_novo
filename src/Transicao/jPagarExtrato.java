...
public class jPagarExtrato extends javax.swing.JPanel {
    ...
    private void jfmVRCHFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_jfmVRCHFocusLost
        String stVRCH = jfmVRCH.getText(); if (stVRCH == null) stVRCH = "0";
        float tVRCH = LerValor.StringToFloat(stVRCH);
        if (tVRCH < 0) {
            // _ Modificado 09-08-2021
            Clear();
            LimpaTransicao();
            Cancelar();
            jTROCO.setValue(0);
            jRESTO.setValue(vrAREC);                
            return;
        }
        
        jbtLancar.setEnabled(tVRCH >= 0);
    ...
    private void jfmVRDNFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_jfmVRDNFocusLost
        if (LerValor.StringToFloat(jfmVRDN.getText()) < 0) {
            // _ Modificado 09-08-2021
            Clear();
            LimpaTransicao();
            Cancelar();
