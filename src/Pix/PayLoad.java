...
import static Bancos.Santander.CalcDig11N;
...
/**
  ...
  public String NossoNumeroPix(String value, int tam) {
     ...
     * @return string[][]
     */
    static public String[][] LerBancoPIX(String banco) {
        DbMain conn = VariaveisGlobais.conexao;
        String dadosPix[][] = null;
        try {
            dadosPix = conn.LerCamposTabela(
    ...
    static public void GravarNnumeroPIX(String nbanco, String Value) {
        ...
        try { dadosBol = conn.LerCamposTabela( new String[] {"nnumero"}, "bancos_pix", "Trim(Upper(banco)) = '" + nbanco.trim().toUpperCase() + "'"); } catch (Exception e) {e.printStackTrace();}
        if (dadosBol != null) {
        ...
        if (Double.valueOf(Value) > oldnnumero) {
            ...
            try { conn.ExecutarComando(sql);} catch (Exception e) {
                JOptionPane.showMessageDialog(null, "Ṇo consegui gravar Nnumero!!!\nContacte o administrador do sistema.\nTel.:(21)97665-9897");
                System.exit(1);
                e.printStackTrace();
            }
        } else {
            JOptionPane.showMessageDialog(null, "Nnumero ṇo coerente!!!\nContacte o administrador do sistema.\nTel.:(21)997665-9897");
            System.exit(1);
        }
    }
