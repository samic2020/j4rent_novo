...
public class jPagtos extends javax.swing.JDialog {
    ...
    public void MontaLista(String contrato, String rgprp) {
        ...
        //             "AND r.tag = 'X' ORDER BY a.rc_aut;"; //r.dtvencimento;";
        String sql = "SELECT r.rgprp, r.rgimv, r.contrato, r.dtvencimento, r.rc_aut autenticacao, r.campo, " +
                     "r.dtrecebimento, r.rc_aut FROM auxiliar r WHERE " +
                     "r.conta = 'REC' AND r.rgprp = '" + rgprp + "' AND r.contrato = '" + contrato + "' " +
                     "ORDER BY YEAR(r.dtvencimento), MONTH(r.dtvencimento), DAY(r.dtvencimento);"; //r.rc_aut;";
...
//                     "AND r.tag = 'X' AND r.dtvencimento = '" + Dates.DateFormata("yyyy-MM-dd", Dates.StringtoDate(vecto, "dd/MM/yyyy")) + "' ORDER BY r.dtvencimento;";        
        String sql = "SELECT a.rgprp, a.rgimv, a.contrato, a.dtvencimento, a.campo, " +
                     "a.dtrecebimento, a.rc_aut FROM auxiliar a WHERE a.conta = 'REC' AND " +
                     "a.rgprp = '" + rgprp + "' AND a.contrato = '" + contrato + "' " +
                     "AND a.dtvencimento = '" + Dates.DateFormata("yyyy-MM-dd", Dates.StringtoDate(vecto, "dd/MM/yyyy")) + "' ORDER BY a.dtvencimento;";        
        ...
        try {
            if (pResult.first()) {
                ...
                rcampo = campo;
                mCartVazio = (rcampo.trim().equalsIgnoreCase(""));
            }
        } catch (SQLException ex) {
            rcampo = "";
            ex.printStackTrace();
...
//                     "AND a.contrato = r.contrato AND a.rc_aut = r.autenticacao " +
//                     "AND a.conta = 'REC' AND r.rgprp = '" + rgprp + "' AND r.contrato = '" + contrato + "' " +
//                     "AND r.tag = 'X' AND r.dtvencimento = '" + Dates.DateFormata("yyyy-MM-dd", Dates.StringtoDate(vecto, "dd/MM/yyyy")) + "' ORDER BY a.rc_aut;"; // r.dtvencimento;";
//////        String sql = "SELECT a.rgprp, a.rgimv, a.contrato, a.dtvencimento, a.campo, " +
//////                     "a.dtrecebimento, a.rc_aut FROM auxiliar a WHERE a.conta = 'REC' " +
//////                     "AND a.rgprp = '" + rgprp + "' AND a.contrato = '" + contrato + "' " +
//////                     "AND a.dtvencimento = '" + Dates.DateFormata("yyyy-MM-dd", Dates.StringtoDate(vecto, "dd/MM/yyyy")) + "' ORDER BY a.rc_aut;"; // r.dtvencimento;";
        String sql = "SELECT a.rgprp, a.rgimv, a.contrato, a.dtvencimento, a.campo, " +
                     "(SELECT e.dtrecebimento FROM extrato e WHERE e.rc_aut = a.autenticacao LIMIT 1), a.autenticacao rc_aut FROM recibo a WHERE " +
                     "a.rgprp = '" + rgprp + "' AND a.contrato = '" + contrato + "' " +
                     "AND a.dtvencimento = '" + Dates.DateFormata("yyyy-MM-dd", Dates.StringtoDate(vecto, "dd/MM/yyyy")) + "' ORDER BY a.autenticacao;"; // r.dtvencimento;";
                //"SELECT * FROM auxiliar WHERE conta = 'REC' AND rgprp = '" + rgprp + "' AND contrato = '" + contrato + "' AND dtvencimento = '" + Dates.DateFormata("yyyy-MM-dd", Dates.StringtoDate(vecto, "dd/MM/yyyy")) + "';";
        ResultSet pResult = conn.AbrirTabela(sql, ResultSet.CONCUR_UPDATABLE);
