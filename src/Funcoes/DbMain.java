...
public class DbMain {
    ...
    private String password = "7kf51b"; //"a7v600x";
    ...
    */
    ...
    public int ExecutarComando(String sqlString, boolean supressErros) {
        int hRetorno = 0;
        Connection connectionSQL = this.conn;
        Statement stm = null;
        try {
        ...
        try {
            ...
            if (!supressErros) ex.printStackTrace();
        }
        return hRetorno;
    }
    ...
    public void CriarMySqlProcedures(String qual, String cParam) {
        ...
        if ("ALL".equals(qual.toUpperCase()) || "GERAMOVTO".equals(qual.toUpperCase())) {
            ...
            sql = "CREATE DEFINER=`root`@`localhost` PROCEDURE  `" + dataBaseName + "`.`GeraMovto`() BEGIN " +
                 ...
                 "END LOOP MLOOP;" + 
                 "CLOSE cur1;" + 
                 "INSERT INTO RECIBO SELECT * FROM TMPRECIBO; DELETE FROM TMPRECIBO;" + 
                 "END";   
            ExecutarComando(sql);
        }
        */
    ...
    public boolean ChecaFuncoesDb() {
        String f_AtRemove = "CREATE DEFINER=`root`@`%` FUNCTION `" + dataBaseName + "`.`AtRemove`(sCampo LONGTEXT) RETURNS longtext CHARSET latin1 \n" +
        "BEGIN \n" +
        "	DECLARE iPos Int; \n" +
        "	DECLARE sCp LONGTEXT; \n" +
        "	DECLARE sCpos LONGTEXT; \n" +
        ...
        String f_AtUnUpgrade = "CREATE DEFINER=`root`@`%` FUNCTION `" + dataBaseName + "`.`AtUnUpgrade`(sCampo LONGTEXT) RETURNS tinyint(1) \n" +
        "BEGIN \n" +
        "	DECLARE iPos Int;  \n" +
        "	DECLARE sCp LONGTEXT;  \n" +
        "	DECLARE sAux LONGTEXT;  \n" +
        ...
        String f_CotaParc = "CREATE DEFINER=`root`@`%` FUNCTION `" + dataBaseName + "`.`CotaParc`(sCampo LONGTEXT) RETURNS longtext CHARSET latin1 \n" +
        "BEGIN  \n" +
        "	DECLARE iPos Int;  \n" +
        "	DECLARE sCp LONGTEXT;  \n" +
        "	DECLARE sCpos LONGTEXT;  \n" +
        ...
        String f_CountStr = "CREATE DEFINER=`root`@`%` FUNCTION `" + dataBaseName + "`.`CountStr`(cCampo LongText, patern VarChar(30)) RETURNS int(11) \n" +
        "BEGIN declare i Int; declare nStr Int; declare nRet Int; set i = 1; set nStr = LENGTH(cCampo); set nRet = 0; WHILE i <= nStr DO IF Mid(cCampo, i, 1) = Trim(patern) THEN set nRet = nRet + 1; END IF; set i = i + 1; END WHILE; RETURN nRet; END;";

        String f_CriptaNome = "CREATE DEFINER=`root`@`%` FUNCTION `" + dataBaseName + "`.`CriptaNome`(sValue text) RETURNS text CHARSET latin1 \n" +
        "BEGIN \n" + 	
        "	declare i Int; \n" + 	
        "    declare iLen Int; \n" + 	
        "    declare sRet text; \n" + 	
        ...
        String f_DecriptaNome = "CREATE DEFINER=`root`@`%` FUNCTION `" + dataBaseName + "`.`DecriptaNome`(sValue text) RETURNS text CHARSET latin1 \n" +
        "BEGIN \n" +	
        "	declare i Int; \n" +  	
        "    declare iLen Int; \n" + 	
        "    declare sRet text; \n" + 	
        ...
        String f_gDesconto = "CREATE DEFINER=`root`@`%` FUNCTION `" + dataBaseName + "`.`gDesconto`(sContrato CHAR(6), sReferencia CHAR(7)) RETURNS longtext CHARSET latin1 \n" +
        "BEGIN \n" + 	
        "	DECLARE a LONGTEXT;  \n" +	
        "	DECLARE i Int; \n" + 	
        "	DECLARE b LONGTEXT; \n" + 	
        "	DECLARE done BOOLEAN DEFAULT FALSE; \n" + 	
        "	DECLARE cur1 CURSOR FOR SELECT CONCAT( 'DC:2:',valor,':','0000',':DC:',sigla,':DS',CriptaNome(descricao)) AS campo FROM Descontos WHERE contrato = sContrato and referencia = sReferencia; \n" + 	
        "	DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE; \n" + 	
        "	set i = 0; \n" + 	
        "	OPEN cur1; \n" + 	
        ...
        "	set b = CONCAT_WS( ';',b,a); \n" + 	
        "	set i = i + 1; \n" + 	
        "	IF i >= ( SELECT COUNT(referencia) FROM Descontos WHERE contrato = sContrato AND referencia = sReferencia) THEN \n" + 		
        "		set done = TRUE; \n" + 	
        "	END IF; \n" + 	
        ...
        String f_gDiferenca = "CREATE DEFINER=`root`@`%` FUNCTION `" + dataBaseName + "`.`gDiferenca`(sContrato CHAR(6), sReferencia CHAR(7)) RETURNS longtext CHARSET latin1 \n" +
        "BEGIN DECLARE a LONGTEXT; DECLARE i Int; DECLARE b LONGTEXT; DECLARE done BOOLEAN DEFAULT FALSE; DECLARE cur1 CURSOR FOR SELECT CONCAT( 'DF:2:',valor,':','0000',':DF:',sigla,':DS',CriptaNome(descricao)) AS campo FROM Diferenca WHERE contrato = sContrato and referencia = sReferencia; DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE; set i = 0; OPEN cur1; REPEAT FETCH cur1 INTO a; set b = CONCAT_WS( ';',b,a); set i = i + 1; IF i >= ( SELECT COUNT(referencia) FROM Diferenca WHERE contrato = sContrato AND referencia = sReferencia) THEN set done = TRUE; END IF ; UNTIL done END REPEAT; CLOSE cur1; RETURN Trim(b); END;";

        String f_gSeguro = "CREATE DEFINER=`root`@`%` FUNCTION `" + dataBaseName + "`.`gSeguro`(sContrato CHAR(6), sReferencia CHAR(7)) RETURNS longtext CHARSET latin1 \n" +
        "BEGIN DECLARE a LONGTEXT; DECLARE i Int; DECLARE b LONGTEXT; DECLARE done BOOLEAN DEFAULT FALSE; DECLARE cur1 CURSOR FOR SELECT CONCAT( 'SG:3:',valor,':','0000',':SG:',sigla) AS campo FROM Seguros WHERE contrato = sContrato and referencia = sReferencia; DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE; set i = 0; OPEN cur1; REPEAT FETCH cur1 INTO a; set b = CONCAT_WS( ';',b,a); set i = i + 1; IF i >= ( SELECT COUNT(referencia) FROM Seguros WHERE contrato = sContrato AND referencia = sReferencia) THEN set done = TRUE; END IF ; UNTIL done END REPEAT; CLOSE cur1; RETURN Trim(b); END;";

        String f_jgeral = "CREATE DEFINER=`root`@`%` FUNCTION `" + dataBaseName + "`.`jgeral`(sValue Text) RETURNS text CHARSET latin1 \n" +
        "BEGIN RETURN RIGHT(sValue,INSTR(REVERSE(sValue),':') -1); END;";

        String f_plusVal = "CREATE DEFINER=`root`@`%` FUNCTION `" + dataBaseName + "`.`plusVal`(sValue Char(10), sTipo Char(3)) RETURNS decimal(10,2) \n" +
        "BEGIN 	declare nRet DECIMAL(10, 2); 	set nRet = StrVal(sValue); 	if sTipo = 'DEB' THEN set nRet = nRet * -1; END IF; 	RETURN nRet; END;";

        String f_ProxVecto = "CREATE DEFINER=`root`@`%` FUNCTION `" + dataBaseName + "`.`ProxVecto`(sCampo LongText, sVecto Char(10)) RETURNS char(10) CHARSET latin1 \n" +
        "BEGIN \n" + 
        "	declare ultDiaMes Int;  \n" +
        "	declare iPos Int;  \n" +
        "	declare iDay Int;  \n" +
        ...
        "	declare iMes Int;  \n" +
        "	declare iAno Int;  \n" +
        "	set iMes = CAST(Mid(sVecto, 4, 2) AS UNSIGNED Int);  \n" +
        "	set iANO = CAST(Mid(sVecto, 7, 4) AS UNSIGNED Int);  \n" +
        "	set iMes = iMes + 1;  \n" +
        "	IF iMes > 12 THEN set iMes = 1;  \n" +
        "	set iAno = iAno + 1; END IF;  \n" +
        "	set sMes = CAST(CONCAT(Right(CONCAT('0000',TRIM(CAST(iAno AS CHAR(4)))),4),'-',RIGHT(CONCAT('00',TRIM(CAST(iMes AS CHAR(2)))),2),'-01') AS DATE); \n" +
        "	set ultDiaMes = Day(LAST_DAY(sMes)); \n" + 
        "	set iPos = InStr(Trim(sCampo), 'DV');  \n" +
        "	IF iPos > 0 THEN  \n" +
        ...
        "		END IF;  \n" +
        "	ELSE  \n" +
        "		set iDay = CAST(LEFT(sVecto,2) AS UNSIGNED Int);  \n" +
        "		IF iDay > ultDiaMes THEN  \n" +
        "			set rDay = ultDiaMes;  \n" +
        "		ELSE  \n" +
        "			set rDay = iDay;  \n" +
        "		END IF;  \n" +
        "	END IF;  \n" +
        "	RETURN CONCAT(Right(CONCAT('00',TRIM(CAST(rDay AS CHAR(2)))),2),'/',RIGHT(CONCAT('00',TRIM(CAST(iMes AS CHAR(2)))),2),'/',RIGHT(CONCAT('0000',TRIM(CAST(iAno AS CHAR(4)))),4));  \n" +
        "END;";

        String f_ProxVecto2 = "CREATE DEFINER=`root`@`%` FUNCTION `" + dataBaseName + "`.`ProxVecto2`(sCampo LongText, sVecto Char(10), sVecto2 Char(10)) RETURNS char(10) CHARSET latin1 \n" +
        "BEGIN \n" +
        "	declare ultDiaMes Int;  \n" +
        "	declare iPos Int;  \n" +
        "	declare iDay Int;  \n" +
        ...
        String f_RCampos = "CREATE DEFINER=`root`@`%` FUNCTION `" + dataBaseName + "`.`RCampos`(sValue CHAR(2)) RETURNS longtext CHARSET latin1 \n" +
        "BEGIN \n" +
        "	DECLARE sRet   longtext; \n" +
        "	DECLARE sBusc1 longtext; \n" +
        "	DECLARE sBusc2 longtext; \n" +
        "	set sBusc1 = (SELECT Upper(a.DESCR) AS descricao FROM ADM a WHERE a.CODIGO = sValue); \n" +
        "	IF IsNull(sBusc1) THEN \n" +
        "		set sBusc2 = (SELECT Upper(l.CART_DESCR) FROM LANCART l WHERE l.CART_CODIGO = sValue); \n" +
        "		IF IsNull(sBusc2) THEN \n" +
        "			IF sValue = 'DF' THEN set sRet = 'DIFERENCA'; END IF; \n" +
        "			IF sValue = 'DC' THEN set sRet = 'DESCONTO'; END IF; \n" +
        "		ELSE \n" +
        "			set sRet = sBusc2; \n" +
        "		END IF; \n" +
        "	ELSE \n" +
        ...
        String f_RetAvDataRid2 = "CREATE DEFINER=`root`@`%` FUNCTION `" + dataBaseName + "`.`RetAvDataRid2`(sValue Text) RETURNS date \n" +
        "BEGIN declare i Int; declare iLen Int; declare iPto Int; declare lRet CHAR(8); declare bLog Boolean; set iLen = LENGTH(UCase(Trim(sValue))); set i = 1; set iPto = 0; set bLog = TRUE; IF iLen > 0 THEN   WHILE bLog DO     IF i >= iLen THEN       set bLog = FALSE;     END IF;     IF iPto = 7 THEN       set bLog = FALSE;     END IF;     IF MID(sValue, i, 1) = ':' THEN       set iPto = iPto + 1;     END IF;     set i = i + 1;   END WHILE;   set lRet = MID(sValue,i - 1,8); END IF; RETURN CAST(CONCAT(MID(lRet, 5, 4), '-', MID(lRet, 3, 2), '-', MID(lRet, 1, 2)) AS DATE); END;";

        String f_RetAvDescRid2 = "CREATE DEFINER=`root`@`%` FUNCTION `" + dataBaseName + "`.`RetAvDescRid2`(sValue Text) RETURNS text CHARSET latin1 \n" +
        "BEGIN declare i Int; declare iLen Int; declare iPto Int; declare lRet Text; declare bLog Boolean; set iLen = LENGTH(UCase(Trim(sValue))); set i = 1; set iPto = 0; set bLog = TRUE; IF iLen > 0 THEN   WHILE bLog DO     IF i >= iLen THEN       set bLog = FALSE;     END IF;     IF iPto = 10 THEN       set bLog = FALSE;     END IF;     IF MID(sValue, i, 1) = ':' THEN       set iPto = iPto + 1;     END IF;     set i = i + 1;   END WHILE;   set lRet = MID(sValue,i - 1); END IF; RETURN MID(lret,1,InStr(lRet,':') - 1); END;";

        String f_RetAvTipoRid2 = "CREATE DEFINER=`root`@`%` FUNCTION `" + dataBaseName + "`.`RetAvTipoRid2`(sValue Text) RETURNS char(3) CHARSET latin1 \n" +
        "BEGIN declare i Int; declare iLen Int; declare iPto Int; declare lRet CHAR(3); declare bLog Boolean; set iLen = LENGTH(UCase(Trim(sValue))); set i = 1; set iPto = 0; set bLog = TRUE; IF iLen > 0 THEN   WHILE bLog DO     IF i >= iLen THEN       set bLog = FALSE;     END IF;     IF iPto = 8 THEN       set bLog = FALSE;     END IF;     IF MID(sValue, i, 1) = ':' THEN       set iPto = iPto + 1;     END IF;     set i = i + 1;   END WHILE;   set lRet = MID(sValue,i - 1,3); END IF; RETURN lRet; END;";

        String f_RetAvUserRid2 = "CREATE DEFINER=`root`@`%` FUNCTION `" + dataBaseName + "`.`RetAvUserRid2`(sValue Text) RETURNS text CHARSET latin1 \n" +
        "BEGIN RETURN RIGHT(sValue,INSTR(REVERSE(sValue),':') -1); END;";

        String f_RetAvValorRid2 = "CREATE DEFINER=`root`@`%` FUNCTION `" + dataBaseName + "`.`RetAvValorRid2`(sValue Text) RETURNS decimal(10,2) \n" +
        "BEGIN declare i Int; declare iLen Int; declare iPto Int; declare lRet CHAR(10); declare bLog Boolean; set iLen = LENGTH(UCase(Trim(sValue))); set i = 1; set iPto = 0; set bLog = TRUE; IF iLen > 0 THEN   WHILE bLog DO     IF i >= iLen THEN       set bLog = FALSE;     END IF;     IF iPto = 2 THEN       set bLog = FALSE;     END IF;     IF MID(sValue, i, 1) = ':' THEN       set iPto = iPto + 1;     END IF;     set i = i + 1;   END WHILE;   set lRet = MID(sValue,i - 1,10); END IF; RETURN CAST(CAST(MID(lRet, 1, 8) AS UNSIGNED INT) + (CAST(MID(lRet, 9, 2) AS UNSIGNED INT) / 100) AS DECIMAL(10,2)); END;";

        String f_StrDate = "CREATE DEFINER=`root`@`%` FUNCTION `" + dataBaseName + "`.`StrDate`(value VARCHAR(10)) RETURNS date \n" +
        "BEGIN \n" +
        "	declare dia CHAR(2); \n" +
        "	declare mes CHAR(2); \n" +
        "	declare ano CHAR(4); \n" +
        "	set dia = MID(value, 1, 2); \n" +
        "	set mes = MID(value, 4, 2); \n" +
        "	set ano = MID(value, 7); \n" +
        "RETURN CAST(CONCAT(ano,'-',mes,'-',dia) AS DATE); \n" +
        "END;";

        String f_StrVal = "CREATE DEFINER=`root`@`%` FUNCTION `" + dataBaseName + "`.`StrVal`(sValue Char(10)) RETURNS decimal(10,2) \n" +
        "BEGIN declare nRet DECIMAL(10, 2); set nRet = CAST(CONCAT(CAST(Mid(sValue, 1, 8) AS UNSIGNED Int), '.',RIGHT(sValue,2)) AS DECIMAL(10,2)); RETURN nRet; END;";

        String f_ValStr = "CREATE DEFINER=`root`@`%` FUNCTION `" + dataBaseName + "`.`ValStr`(iValue DECIMAL(10,2)) RETURNS char(10) CHARSET latin1 \n" +
        "BEGIN declare iValor CHAR(20); declare iCento CHAR(4); set iValor = Right(CONCAT('00000000',CAST(TRUNCATE(iValue,0) AS CHAR(10))),8); set iCento = CAST(((iValue - TRUNCATE(iValue, 0)) * 100) AS CHAR); RETURN CONCAT(iValor, iCento); END;";
        ...
        for (String f : f_funcoes) {
            if (ExecutarComando(f, true) < 0) {
                msg += f + " \n";
                retorno = false;
            };
        }
    ...
    public void BancoPix() {
        String createSQL = "CREATE TABLE IF NOT EXISTS `" + dataBaseName + "`.`bancos_pix` (`id` int(11) NOT NULL AUTO_INCREMENT, `tipo` varchar(10) COLLATE utf8_bin DEFAULT NULL, `chave` varchar(45) COLLATE utf8_bin DEFAULT NULL, `banco` varchar(45) COLLATE utf8_bin DEFAULT NULL, `nnumero` int NOT NULL DEFAULT '0', PRIMARY KEY (`id`)) ENGINE=MyISAM DEFAULT CHARSET=utf8 COLLATE=utf8_bin;";
...
//  `DTRECEBIMENTO` date NOT NULL DEFAULT '0000-00-00',
//  `RC_AUT` double NOT NULL DEFAULT '0',
//  `EXTORNADO` varchar(1) NOT NULL DEFAULT '',
//  PRIMARY KEY (`AUTOID`)
//) ENGINE=MyISAM AUTO_INCREMENT=763262 DEFAULT CHARSET=utf8;
