...
public class jContasControle extends javax.swing.JInternalFrame {
    ...
    private void initComponents() {
        ...
        jPanel4.setBorder(javax.swing.BorderFactory.createTitledBorder("Seleção/Criação de Contas"));
                    ...
                    .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, layout.createSequentialGroup()
                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addGroup(layout.createSequentialGroup()
                                ...
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                .addComponent(jBuscaPasta, javax.swing.GroupLayout.PREFERRED_SIZE, 279, javax.swing.GroupLayout.PREFERRED_SIZE))
    ...
    private void jbtdelgrupoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbtdelgrupoActionPerformed
        ...
        conn.Auditor("CONTAS CTL/DELETE", "CODIGO: " + id);
        jbtdelgrupo.setEnabled(false);
...
//            }
    ...
    private void FillContas(String sID) {
        ...
        double fTotCred = 0; double fTotDeb = 0; 
        String sql = FuncoesGlobais.Subst("SELECT RetAvDataRid2(campo) data, DecriptaNome(RetAvDescRid2(campo)) descr, RetAvTipoRid2(campo) tipo, RetAvValorRid2(campo) valor, autenticacao FROM avisos WHERE registro = '&1.' AND rid = '5' AND (tag <> 'X' OR ISNULL(tag));", new String[] {sID});
        ...
        try {
            while (hrs.next()) {
                String sinq = null;
                String sData = null;
                try {sinq = hrs.getString("descr");} catch (SQLException ex) {System.out.println("erro sinq");}
                try {sData = Dates.DateFormata("dd/MM/yyyy", hrs.getDate("data"));} catch (SQLException ex) {System.out.println("erro sData");}
                String sAut = null;
                if (hrs.getString("autenticacao") != null) sAut = "" + hrs.getString("autenticacao");
                String sTipo = null;
                try { sTipo = hrs.getString("tipo");} catch (SQLException ex) {System.out.println("erro sTipo");}
                double sValor = 0;
                try { sValor = hrs.getBigDecimal("valor").doubleValue(); } catch (SQLException ex) {System.out.println("erro sValor");}
                String tCred = ""; String tDeb = "";
                if (sTipo.equalsIgnoreCase("CRE")) {
                    tCred = LerValor.doubleToCurrency(sValor,2);
                    fTotCred += sValor;
                    tDeb =  "";
                } else {
                    tCred = "";
                    tDeb = LerValor.doubleToCurrency(sValor, 2);
                    fTotDeb += sValor;
                }
                ...
                if (!"".equals(sinq.trim())) {
                    String aLinhas[] = null;
                    try { aLinhas = WordWrap.wrap(sinq, 237, getFontMetrics(new java.awt.Font("SansSerif",Font.NORMAL,8))).split("\n");} catch (Exception ew) {ew.printStackTrace();}
                    if (aLinhas == null || aLinhas.length == 0) {
                        System.out.println("erro alinhas");
                    }
                    for (int k=0;k<=aLinhas.length-1;k++) {
                        String tData = ""; String tDesc = aLinhas[k].replace("ò", " "); String tAut = "";
                        if (k > 0) { 
                            tData = ""; tCred = ""; tDeb = ""; tAut = "";                             
                        } else {
                            tData = sData; 
                            tAut = sAut;
                        }
        ...
        tCredito.setText(LerValor.doubleToCurrency(fTotCred, 2));
        tDebito.setText(LerValor.doubleToCurrency(fTotDeb, 2));
        double sld = fTotCred - fTotDeb;
        tSaldo.setText(LerValor.doubleToCurrency(sld, 2));
