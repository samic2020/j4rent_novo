 ...
 * jAtuTaxas.java
 *
 * Created on 27/03/2012, 10:05:47
 */
...
public class jAtuTaxas extends javax.swing.JInternalFrame {
    ...
    private void ListarContas() {
        ...
        ResultSet rs = conn.AbrirTabela("SELECT * FROM ataxas ORDER BY idataxas;", ResultSet.CONCUR_READ_ONLY);
        try {
            while (rs.next()) {
    ...
    private void initComponents() {
        ...
        jPanel1.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createEtchedBorder(), "Leitura", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("SansSerif", 1, 12), new java.awt.Color(0, 0, 153))); // NOI18N
    ...
    private void jtxLeitorFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_jtxLeitorFocusLost
        ...
        if (lCampos.length > 0) {
            String[][] cCampos = null;
            try {
                cCampos = conn.LerCamposTabela(new String[] {"CART_DESCR"}, "LANCART", "CART_CODIGO = '" + lCampos[0][3] + "'");
            } catch (Exception e) {}

            jCodLeitura.setText(lCampos[1][3]);
            jDescLeitura.setText(cCampos[0][3]);  //lCampos[0][3]);
            codigo_carteira = lCampos[0][3];
            ...
            if (lCampos[4][3] != null) {
                if (!lCampos[4][3].equals("0000")) {
                    String dFormat = lCampos[5][3]; dFormat = (dFormat == null ? "" : dFormat);
                    int tmpV = 0;
        ...
        try {
            ...
            if (rows > 0) {
                ...
                while (rs.next()) {
                    ...
                    String[][] _Carteira = conn.LerCamposTabela(new String[] {"contrato", "campo"}, "CARTEIRA", "rgimv = '" + rgimv + "'");
                    String campo = _Carteira[1][3];
                    ...
                    if (eAT) {
                        ...
                        String[][] _Antecp = conn.LerCamposTabela(new String[] {"contrato"}, "ANTECIPADOS", 
                    ...
                    String[][] _RECIBOPG = conn.LerCamposTabela(new String[] {"contrato","campo","rgprp","rgimv"}, "RECIBO", "contrato = '" + contrato + "' AND tag = 'X' AND " +
                                         "SUBSTRING(dtvencimento,1,7) = '" + Dates.DateFormata("yyyy-MM", dtLeitura) + "';");
                    if (_RECIBOPG != null) {
                        this.contrato = _RECIBOPG[0][3];
                        String wcampo = _RECIBOPG[1][3];
                        ...
                        if (npos > -1) {
                            ...
                            if (difevalor > 0) {
                                // desconto
                                String[][] _LanCart = conn.LerCamposTabela(new String[] {"cart_codigo","cart_ordem","cart_retencao","cart_descr"}, "LANCART", "cart_codigo = '" + _Busca.substring(0,2) + "'");
                                str = _LanCart[3][3].trim();
                                mTipo = "";
                    ...
                    if (_RECIBO != null) {
                        ...
                        if (npos > -1) {
                            ...
                            String[][] _LanCart = conn.LerCamposTabela(new String[] {"cart_codigo","cart_ordem","cart_retencao"}, "LANCART", "cart_codigo = '" + _Busca.substring(0,2) + "'");
                            String[] _campo = new String[] {_LanCart[0][3],_LanCart[1][3],FuncoesGlobais.GravaValor(LerValor.FloatToString(valor/rows)),
                                                            "0000","NT","RZ","ET","IP"};
                        ...
                        if (mCpoDesc.contains("DC:2")) {
                            // desconto
                            String argprp = _RECIBOPG[2][3];
                            String argimv = _RECIBOPG[3][3];
                            String acontrato = _RECIBOPG[0][3];
                        ...
                        try {
                            if (sql != null) conn.ExecutarComando(sql);
                        } catch (Exception e) {}
                    }
                }
