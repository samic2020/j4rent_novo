 ...
 * jRazao.java
 *
 * Created on 03/08/2011, 08:39:41
 */
package Movimento;
    ...
    private void initComponents() {
        ...
        jPanel1Layout.setHorizontalGroup(
            ...
            .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, jPanel1Layout.createSequentialGroup()
                ...
                .addComponent(jDtRazao, javax.swing.GroupLayout.DEFAULT_SIZE, 141, Short.MAX_VALUE)
            ...
            .addGroup(jPanel1Layout.createSequentialGroup()
                ...
                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING, false)
                        .addComponent(jDtRazao, javax.swing.GroupLayout.Alignment.LEADING, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
    ...
    private void printRazao() {
        ...
        SwingWorker worker = new SwingWorker(){  
               ...
               protected Object doInBackground() throws Exception {  
                   int totalFiles = 26; int nFile = 1;
                   jDtRazao.setEnabled(false); jButton1.setEnabled(false);
                   
                    processo.setText("Avisos de Disponivel Corrente");
                    jbarra1.setValue((100 * nFile++) / totalFiles);
                    try {Thread.sleep(1000);} catch (InterruptedException e) {}
                    float[] pAVIDISP = fAVI_DISP();

                    processo.setText("Saldo Proprietario final");
                    jbarra1.setValue((100 * nFile++) / totalFiles);
                    try {Thread.sleep(1000);} catch (InterruptedException e) {}
                    float[] sdProd = SaldosProp();
                    
                    processo.setText("Saldo Proprietario dia Credito");
                    jbarra1.setValue((100 * nFile++) / totalFiles);
                    try {Thread.sleep(1000);} catch (InterruptedException e) {}
                    float[] sdProdDiaCRE = SaldosPropDiaCRE(Dates.DateFormata("yyyy/MM/dd", jDtRazao.getDate()));
                    
                    processo.setText("Saldo Proprietario dia Debito");
                    jbarra1.setValue((100 * nFile++) / totalFiles);
                    try {Thread.sleep(1000);} catch (InterruptedException e) {}
                    float[] sdProdDiaDEB = SaldosPropDiaDEB(Dates.DateFormata("yyyy/MM/dd", jDtRazao.getDate()));
                    ...
                    jbarra1.setValue((100 * nFile++) / totalFiles);
                    try {Thread.sleep(1000);} catch (InterruptedException e) {}
                    iniDate = Dates.DateFormata("yyyy/MM/dd", jDtRazao.getDate()).substring(0, 8) + "01";
                    where = "data between '&1.' AND '&2.'";
                    where = FuncoesGlobais.Subst(where, new String[] {Dates.DateFormata("yyyy/MM/dd", Dates.StringtoDate(iniDate, "yyyy/MM/dd")), Dates.DateFormata("yyyy/MM/dd", Dates.DateAdd(Dates.DIA, -1, jDtRazao.getDate()))});
                    float pDES_ANT = fDES();
                    iniDate = Dates.DateFormata("yyyy/MM/dd", jDtRazao.getDate()).substring(0, 8) + "01";
                    where = "AND (dtrecebimento >= '&1.' AND dtrecebimento < '&2.')";
                    where = FuncoesGlobais.Subst(where, new String[] {Dates.DateFormata("yyyy/MM/dd", Dates.StringtoDate(iniDate, "yyyy/MM/dd")), Dates.DateFormata("yyyy/MM/dd", jDtRazao.getDate())});
                    ...
                    Bean1.set_dsLinha01("PROPRIETARIOS POSITIVOS");
                    float sdprcr = sdProdDiaCRE[0];
                    float sdprdb = sdProdDiaDEB[0];
                    Bean1.set_crLinha01(LerValor.floatToCurrency(sdprcr, 2));
                    Bean1.set_dbLinha01(LerValor.floatToCurrency(sdprdb, 2));
                    Bean1.set_sdLinha01(LerValor.floatToCurrency(sdprcr - sdprdb, 2));
                    sd_cr = sdProd[0];
                    sd_db = (sdprcr - sdprdb);
                    Bean1.set_saLinha01(LerValor.floatToCurrency(sd_cr - sd_db, 2)); 
                    float sdprsf = sdProd[0];
                    Bean1.set_sfLinha01(LerValor.floatToCurrency(sdprsf, 2));
                    lista.add(Bean1);

                    float sdc_cr =0; float sdc_db = 0;
                    Bean1 = new Razao();
                    Bean1.set_dsLinha01("CONDOMINIOS");
                    float sdcprcr = sdProdDiaCRE[2];
                    float sdcprdb = sdProdDiaDEB[2];
                    Bean1.set_crLinha01(LerValor.floatToCurrency(sdcprcr, 2));
                    Bean1.set_dbLinha01(LerValor.floatToCurrency(sdcprdb, 2));
                    Bean1.set_sdLinha01(LerValor.floatToCurrency(sdcprcr - sdcprdb, 2));
                    sdc_cr = sdProd[2];
                    sdc_db = (sdcprcr - sdcprdb);
                    Bean1.set_saLinha01(LerValor.floatToCurrency(sdc_cr - sdc_db, 2)); 
                    float sdcprsf = sdProd[2];
                    Bean1.set_sfLinha01(LerValor.floatToCurrency(sdcprsf, 2));
                    lista.add(Bean1);
                    ...
                    Bean1.set_sdLinha01(LerValor.floatToCurrency(pAVISOC[2], 2));

//                    float[] sldant = fAVI_SOCANT();
////                    sd_cr = pAVISOC_ANT[0] + sldant[0];
////                    sd_db = pAVISOC_ANT[1] + sldant[1];
//                    sd_cr = sldant[0];
//                    sd_db = sldant[1];
                    Bean1.set_saLinha01(LerValor.floatToCurrency(pAVISOC[3], 2));
                    Bean1.set_sfLinha01(LerValor.floatToCurrency(pAVISOC[5], 2));
                    lista.add(Bean1);
                    // acumula sfinal
                    sfinal += pAVISOC[5];
...
//                    Bean1.set_crLinha01(LerValor.floatToCurrency(pAVIRET[0].floatValue(), 2));
//                    Bean1.set_dbLinha01(LerValor.floatToCurrency(pAVIRET[1].floatValue(), 2));
//                    Bean1.set_sdLinha01("-");
//                    Bean1.set_saLinha01("-");
//                    Bean1.set_sfLinha01(LerValor.floatToCurrency(pAVIRET[2].floatValue(), 2));
//                    lista.add(Bean1);

//                    Bean1 = new Razao();
//                    Bean1.set_dsLinha01("CTAS CONTROLE");
//                    Object[] valor_cta_ctr = FCtasControle();
//                    Bean1.set_crLinha01(LerValor.floatToCurrency((float)valor_cta_ctr[0],2));
//                    Bean1.set_dbLinha01(LerValor.floatToCurrency((float)valor_cta_ctr[1],2));        
//                    Bean1.set_sdLinha01(LerValor.floatToCurrency((float)valor_cta_ctr[2],2));
                    //sd_cr = valor_exp_ant;
                    //sd_db = 0;
//                    float tsdDia = (float)valor_cta_ctr[2];
//                    float tcrAcum = (float)valor_cta_ctr[3];
//                    float tdbAcum = (float)valor_cta_ctr[4];
//                    float tsdAcum = tcrAcum - tdbAcum;
//                    float tsdGeral = 0;
//                    if (tsdDia < 0) {
//                        tsdGeral = tsdAcum - Math.abs(tsdDia);
//                    } else {
//                        tsdGeral = tsdAcum + Math.abs(tsdDia);
//                    }
//                    Bean1.set_saLinha01(LerValor.floatToCurrency(tsdAcum, 2));
//                    Bean1.set_sfLinha01(LerValor.floatToCurrency(tsdGeral, 2));
//                    lista.add(Bean1);
...
//                    sfinal += pAVIRET[0] - pAVIRET[1] + (sd_cr - sd_db);
    ...
    private Object[][] fADM() {
        ...
        int fim = DbMain.RecordCount(rs); int posit = 1;
...
//                try {Thread.sleep(1000);} catch (InterruptedException e) {}
    ...
    private Object[][] fDES_OLD() {
        ...
        String sql = "SELECT SUM(valor) FROM jgeral.despesas WHERE " + where + " ORDER BY autentica;";
        ResultSet rs = conn.AbrirTabela(sql, ResultSet.CONCUR_READ_ONLY);
        int fim = DbMain.RecordCount(rs); int posit = 1;
...
//                try {Thread.sleep(1000);} catch (InterruptedException e) {}
                ...
                try { nmeCpo = conn.LerCamposTabela(new String[] {"buffer"}, "COMBOBUFFER", "autoid = '" + scontrato + "'")[0][3];} catch (Exception e) {}
                if (nmeCpo != null) {
                    String subCpo1 = auxMtz[2];
                    float subCpo2 = LerValor.FloatNumber(subCpo1, 2);
    ...
    private float fDES() {
        String sql = "SELECT Sum(valor) total FROM jgeral.despesas WHERE " + where + " ORDER BY autentica;";
        ResultSet rs = conn.AbrirTabela(sql, ResultSet.CONCUR_READ_ONLY);
        int fim = DbMain.RecordCount(rs); int posit = 1;
...
//                try {Thread.sleep(1000);} catch (InterruptedException e) {}
    ...
    private float[] fAVI_RET() {
        ...
        //String sql = "SELECT r.campo FROM retencao r WHERE r.marca <> 'X';";
        ResultSet rs = conn.AbrirTabela(sql, ResultSet.CONCUR_READ_ONLY);
        int fim = DbMain.RecordCount(rs); int posit = 1;
        ...
        ResultSet rs2 = conn.AbrirTabela(sql2, ResultSet.CONCUR_READ_ONLY);
        ...
        DbMain.FecharTabela(rs2);
        
        return new float[] {fCR, fDB};
    }

    private BigDecimal[] fAVI_RET2() {
        // Retenções não pagas
        String sql = "SELECT Sum(RetAvValorRid2(r.campo)) valor FROM retencao r WHERE r.marca != 'X';";
        ResultSet rs = conn.AbrirTabela(sql, ResultSet.CONCUR_READ_ONLY);
        int fim = DbMain.RecordCount(rs); int posit = 1;
        ...
        try {
            while (rs.next()) {
                ...
                BigDecimal auxCpo = rs.getBigDecimal("valor");
                fDb = fDb.add(auxCpo);
            }
        } catch (SQLException ex) {}
        DbMain.FecharTabela(rs);
          ...
          // Retenções pagas
        sql = "SELECT Sum(RetAvValorRid2(r.campo)) valor FROM retencao r WHERE r.marca = 'X' AND rt_pagamento = '&1.';";
        sql = FuncoesGlobais.Subst(sql, new String[] {Dates.DateFormata("yyyy/MM/dd", jDtRazao.getDate())});
        rs = conn.AbrirTabela(sql, ResultSet.CONCUR_READ_ONLY);
        fim = DbMain.RecordCount(rs); posit = 1;
        
        BigDecimal fDb2 = new BigDecimal("0");
        ...
        try {
            while (rs.next()) {
                ...
                BigDecimal auxCpo = rs.getBigDecimal("valor");
                if (auxCpo != null) fDb2 = fDb2.add(auxCpo);
            }
        } catch (SQLException ex) {}
        DbMain.FecharTabela(rs);
      
          // Retenções do dia
        sql = "SELECT * FROM extrato WHERE DTRECEBIMENTO = '&1.' AND INSTR(CAMPO,'RT');";
        sql = FuncoesGlobais.Subst(sql, new String[] {Dates.DateFormata("yyyy/MM/dd", jDtRazao.getDate())});
        rs = conn.AbrirTabela(sql, ResultSet.CONCUR_READ_ONLY);
        fim = DbMain.RecordCount(rs); posit = 1;
        ...
        try {
            while (rs.next()) {
                ...
                if (pos > -1) {
                    String auxMtz[] = auxMtz1[pos].split(":");
                    String subCpo1 = auxMtz[2];
                    BigDecimal subCpo2 = new BigDecimal(LerValor.FormatNumber2(subCpo1,2));
        ...
        return new BigDecimal[] {fCR, fDb2, fDb};
    }

    private float[] AVI_SOC() {
        //String selectSQL = "SELECT registro, RetAvDataRid2(campo) as `data`, if(RetAvTipoRid2(campo)='CRE',RetAvValorRid2(campo),0) as credito, if(RetAvTipoRid2(campo)='DEB',RetAvValorRid2(campo),0) as debito FROM jgeral.avisos where rid = '3' AND TAG <> 'X' ORDER BY registro;";
        String selectSQL = "SELECT registro, RetAvDataRid2(campo) as `data`, IF(RetAvTipoRid2(campo) = 'CRE',RetAvValorRid2(campo),0) credito, IF(RetAvTipoRid2(campo) = 'CRE',0, RetAvValorRid2(campo)) debito FROM avisos WHERE rid = '3' AND (tag <> 'X' OR ISNULL(tag));";
        ...
        try {
            while (rs.next()) {
                jbarra2.setValue((100 * posit++)/ fim);
                long diaDiff = Dates.datesBetween(Dates.DIA, jDtRazao.getDate(), rs.getDate("data"));
                if (diaDiff == 0) {
                    dCR += rs.getFloat("credito");
                    dDB += rs.getFloat("debito");
                    continue;
                }
                fCR += rs.getFloat("credito");
                fDB += rs.getFloat("debito");
            }
        } catch (SQLException ex) {}
        DbMain.FecharTabela(rs);
        
        String sociosSQL ="SELECT saldoant FROM jgeral.socios;";
        ResultSet srs = conn.AbrirTabela(sociosSQL, ResultSet.CONCUR_READ_ONLY);
        fim = DbMain.RecordCount(srs); posit = 1;        
        float sdAnt = 0;
        try {
            while (srs.next()) {
                jbarra2.setValue((100 * posit++)/ fim);
                sdAnt += srs.getFloat("saldoant");
            }
        } catch (SQLException ex) {}
        DbMain.FecharTabela(srs);
        
        return new float[] {dCR, dDB, dCR - dDB, fCR + sdAnt - fDB, fDB, fCR + sdAnt - fDB + dCR - dDB};
    }
    
    private float[] fAVI_SOCANT() {
        String sql = "SELECT SUM(if(saldoant >= 0,saldoant,0)) cr, SUM(if(saldoant >= 0,0,saldoant)) db FROM jgeral.socios;";
        ResultSet rs = conn.AbrirTabela(sql, ResultSet.CONCUR_READ_ONLY);
        int fim = DbMain.RecordCount(rs); int posit = 1;
...
//                try {Thread.sleep(1000);} catch (InterruptedException e) {}
    ...
    private float[] CalcRecAux() {
        ...
        int fim = DbMain.RecordCount(imResult); int posit = 1;
        ...
        try {
            while (imResult.next()) {
                ...
                String tinq = null;
                try {tinq = conn.LerCamposTabela(new String[] {"nomerazao"}, "locatarios", "contrato = '" + tcontrato + "'")[0][3];} catch (Exception e) {tinq = null;}
                ...
                //
                try {
                    campo = conn.LerCamposTabela(new String[] {"campo"}, "auxiliar", "Upper(conta) LIKE 'ADM%' AND rgprp = '" + trgprp + "' AND contrato = '" + tcontrato + "' AND rc_aut = '" + imResult.getString("rc_aut") + "'")[0][3];
    ...
    private String BuscaNomeCampo(String campo) {
        ...
        if (LerValor.isNumeric(campo)) {
            try {
                retorno = conn.LerCamposTabela(new String[] {"cart_descr"}, "LANCART", "cart_codigo = '" + campo + "'")[0][3];
            } catch (Exception ex) {
            ...
            if ("DF;DC".contains(campo)) {
                ...
                try {
                    retorno = conn.LerCamposTabela(new String[] {"descr"}, "ADM", "codigo = '" + campo + "'")[0][3];
                } catch (Exception ex) {
                    Logger.getLogger(jRazaoAcumulado.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
    ...
    private Object[][] ListaCamposAdm() {
        Object[][] retorno = {};
        String newWhere = where.replace("dtrecebimento", "RetAvDataRid2(campo)");
        String sSql = "SELECT registro as contrato, campo FROM avisos WHERE rid = 2 " + newWhere + " ORDER BY autenticacao";
    ...
    private Object[] FCtasControle() {
        float fTotCred = 0; float fTotDeb = 0; 
        //String sql = "SELECT campo, autenticacao FROM avisos INNER JOIN combobuffer on registro = autoid WHERE rid = '5' AND (tag <> 'X' OR ISNULL(tag));";
        String sql = "SELECT a.registro, RetAvDataRid2(a.campo) `data`, IF(RetAvTipoRid2(a.campo) = 'CRE', RetAvValorRid2(a.campo),0) AS credito, IF(RetAvTipoRid2(a.campo) = 'DEB', RetAvValorRid2(a.campo),0) AS debito FROM avisos a INNER JOIN combobuffer c on a.registro = c.autoid WHERE a.rid = '5' AND (a.tag <> 'X' OR ISNULL(a.tag));";
        ResultSet hrs = conn.AbrirTabela(sql, ResultSet.CONCUR_READ_ONLY);

        float tcrDia = 0, tdbDia = 0, tsdDia = 0;
        try {
            while (hrs.next()) {
                // Saldo do dia
                Date dtPagto = hrs.getDate("data");
                long diaDiff = Dates.datesBetween(Dates.DIA, jDtRazao.getDate(), dtPagto);
                if (diaDiff == 0) {
                    tcrDia += hrs.getFloat("credito");
                    tdbDia += hrs.getFloat("debito");
                    continue;
                }
                
                fTotCred += hrs.getFloat("credito");
                fTotDeb += hrs.getFloat("debito");
            }
        ...
        tsdDia = tcrDia - tdbDia;
        
        return new Object[] {tcrDia, tdbDia, tsdDia, fTotCred, fTotDeb, sld};
    }

    // Saldo Proprietário Novo Calculo
    private float[] SaldosProp() {
        String sql = "SELECT * FROM proprietarios ORDER BY Lower(nome);";
        ResultSet rst = conn.AbrirTabela(sql, ResultSet.CONCUR_READ_ONLY);
        int fim = DbMain.RecordCount(rst); int posit = 1;
        float sdPos = 0; float sdNeg = 0;
        float scPos = 0; float scNeg = 0;
        try {
            while (rst.next()) {                
                jbarra2.setValue((100 * posit++)/ fim);
                try {Thread.sleep(10);} catch (InterruptedException e) {}
                ...
                String nmprp = rst.getString("nome");
                float saldo =  SaldoProp(rgprp);
                if (!nmprp.toUpperCase().startsWith("COND")) {
                    if (saldo > 0) {
                        sdPos += saldo;
                    } else {
                        sdNeg += saldo;
        ...
        return new float[] {sdPos, sdNeg, scPos, scNeg};                
    }
    
    private float SaldoProp(String jRgprp) {
    ...
    private float[] SaldosPropDiaCRE(String data) {
        String sql = "SELECT * FROM proprietarios ORDER BY Lower(nome);";
        ResultSet rst = conn.AbrirTabela(sql, ResultSet.CONCUR_READ_ONLY);
        int fim = DbMain.RecordCount(rst); int posit = 1;
        float sdPos = 0; float sdNeg = 0;
        ...
        try {
            while (rst.next()) {                
                ...
                String rgprp = rst.getString("rgprp");
                String nmprp = rst.getString("nome");
                float saldo =  SaldoPropDiaCRE(rgprp, data);
                if (!nmprp.toUpperCase().startsWith("COND")) {
                    if (saldo > 0) {
                        sdPos += saldo;
                    } else {
    ...
    private float SaldoPropDiaCRE(String jRgprp, String data) {
        float fTotCred = 0; float fTotDeb = 0; float fTotAdi = 0; float fSaldoAnt = 0;
        try {
            String sdant = conn.LerCamposTabela(new String[] {"saldoant"}, "proprietarios", "rgprp = '" + jRgprp + "'")[0][3];
            fSaldoAnt = Float.valueOf(sdant.trim());
    ...
    private float[] SaldosPropDiaDEB(String data) {
        String sql = "SELECT * FROM proprietarios ORDER BY Lower(nome);";
        ResultSet rst = conn.AbrirTabela(sql, ResultSet.CONCUR_READ_ONLY);
        int fim = DbMain.RecordCount(rst); int posit = 1;
        float sdPos = 0; float sdNeg = 0;
        ...
        try {
            while (rst.next()) {                
                ...
                String rgprp = rst.getString("rgprp");
                String nmprp = rst.getString("nome");
                float saldo =  SaldoPropDiaDEB(rgprp, data);
                if (!nmprp.toUpperCase().startsWith("COND")) {
                    if (saldo > 0) {
                        sdPos += saldo;
                    } else {
    ...
    private float SaldoPropDiaDEB(String jRgprp, String data) {
        float fTotCred = 0; float fTotDeb = 0; float fTotAdi = 0; float fSaldoAnt = 0;
        try {
            String sdant = conn.LerCamposTabela(new String[] {"saldoant"}, "proprietarios", "rgprp = '" + jRgprp + "'")[0][3];
            fSaldoAnt = Float.valueOf(sdant.trim());
