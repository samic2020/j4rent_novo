    ...
    public void ImprimeAdiantamentoPDF(float nAut, String[][] Valores, String texto, String ValorRec, String idNome, String idAviso, String cutPaper) throws SQLException {
        ...
        for (int i=0;i<jRecibos.getRowCount();i++) {
            ...
            if (tag.equalsIgnoreCase("X")) {
                ...
                ResultSet adAviso = conn.AbrirTabela(adSql, ResultSet.CONCUR_READ_ONLY);
                try {
                    while (adAviso.next()) {
...
//                RefreshVisor();
                ...
                new toPrint(FileNamePdf, VariaveisGlobais.AdiantAviso.split(",")[0], VariaveisGlobais.AdiantAviso.split(",")[1], VariaveisGlobais.AdiantAviso.split(",")[2]);
...
//                }
    ...
    public void ImprimeAdiantamentoPDF2(float nAut, String[][] Valores, String texto, String ValorRec, String idNome, String idAviso, String cutPaper) throws SQLException {
        List<Extrato> lista = new ArrayList<Extrato>();
        ...
        for (int i=0;i<jRecibos.getRowCount();i++) {
            ...
            if (tag.equalsIgnoreCase("X")) {
                ...
                // Aqui
                String[][] linhas = Recalcula(jCodigo.getSelectedItem().toString(), trgimv, tcontrato, tvecto);
                for (int nl=0; nl<linhas.length; nl++) {
                    if (linhas[nl][0] == null) continue;
                    
                    if (linhas[nl][0].toString().trim().toLowerCase().equalsIgnoreCase("aluguel")) {
                        sCampos = FuncoesGlobais.ArraysAdd(sCampos, new String[] {linhas[nl][0].toString(),"0;;black",new Pad(linhas[nl][2].toString(),12).LPad() + " ",""});
                    }
                    if (linhas[nl][0].toString().toLowerCase().contains(VariaveisGlobais.dCliente.get("DC").toLowerCase())) {
                        sCampos = FuncoesGlobais.ArraysAdd(sCampos, new String[] {linhas[nl][0].toString(),"0;;black","",new Pad(linhas[nl][2].toString(),12).LPad() + " "});                
                    }
                    if (linhas[nl][0].toString().toLowerCase().contains(VariaveisGlobais.dCliente.get("DF").toLowerCase())) {
                        sCampos = FuncoesGlobais.ArraysAdd(sCampos, new String[] {linhas[nl][0].toString(),"0;;black",new Pad(linhas[nl][2].toString(),12).LPad() + " ",""});                    
                    }
                }
...
//                }
        ...
        for (int i=0;i<jRecibos.getRowCount();i++) {
            ...
            if (tag.equalsIgnoreCase("X")) {
                ...
                for (int n=0; n<linhas.length; n++) {
                    if (linhas[n][0] == null) continue;
                    
                    if (linhas[n][0].toString().trim().toLowerCase().equalsIgnoreCase("aluguel")) {
                        corpo += new Pad(linhas[n][0].toString(),15).RPad() + "R$ " + new Pad(linhas[n][2].toString(),12).LPad() + "(+)\n";
                    }
                    if (linhas[n][0].toString().toLowerCase().contains("desc")) {
                        corpo += new Pad(linhas[n][0].toString(),15).RPad() + "R$ " + new Pad(linhas[n][2].toString(),12).LPad() + "(+)\n";
                    }
                    if (linhas[n][0].toString().toLowerCase().contains("dif")) {
                        corpo += new Pad(linhas[n][0].toString(),15).RPad() + "R$ " + new Pad(linhas[n][2].toString(),12).LPad() + "(+)\n";
                    }
                }
                corpo += new Pad(VariaveisGlobais.dCliente.get("CM"),15).RPad() + "R$ " + new Pad(tcomi,12).LPad() + "(-)\n" +
                         new Pad("Liquido",15).RPad() + "R$ " + new Pad(tvalor,12).LPad() + "(=)\n\n";
            }
        }        
    ...
    public String[][] Recalcula(String rgprp, String rgimv, String contrato, String vencimento) {
        String[][] linhas = null;
        try {
            linhas = MontaRecibo(rgprp, rgimv, contrato, vencimento);
        } catch (Exception ex) {} 

        return linhas;
    }

    public String[][] MontaRecibo(String rgprp, String rgimv, String contrato, String vecto) throws SQLException, ParseException {
...
//            VariaveisGlobais.ccampos = pResult.getString("campo");
            ...
            for (int i=0; i<= a.length() - 1; i++) {
                ...
                if (Campo.length > 0) {
                    //MontaCampos(Campo, i);
                    linhas[i][0] = Campo[0];
                    linhas[i][1] = Campo[Campo.length - 1];
                    linhas[i][2] = Campo[1];
                }
            }
        }
    ...
    public String Imprimir(String corpo) throws FileNotFoundException, IOException, SQLException {
        ...
        for (int a=1; a<=vias; a++) {
            ImprimeAdiantamentoPDF2((float)nAut, aTrancicao, corpo, jValor.getText(), idNome, idAviso, "F");
        }
            ...
            .addGroup(jPanel1Layout.createSequentialGroup()
                ...
                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addGroup(jPanel1Layout.createSequentialGroup()
                        ...
                        .addComponent(delAvAd, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)))
                .addContainerGap())
        );
    ...
    private float[] CalcularRecibo(String rgprp, String rgimv, String contrato, String vecto) {
        ...
        float vcomi = tRecibo * (fComissao / 100);
    ...
    private void jbtPrintDemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbtPrintDemActionPerformed
        ...
        String idNome = ""; String idAviso = ""; String idConta = ""; String rgprp = jCodigo.getSelectedItem().toString();
        idAviso = "DEMONSTRATIVO - ";
        idNome = jCodigo.getSelectedItem().toString() + " - " + jNomeProp.getSelectedItem().toString().trim();
        
        try {ImprimeAdiantamentoPDF2(-1, new String[][] {}, corpo, jValor.getText(), idNome, idAviso, "F");} catch (Exception e) {e.printStackTrace();}
    ...
    public void upgAdAviso(float aut) {
        for (int i=0;i<jRecibos.getRowCount();i++) {
            ...
            if (tag.equalsIgnoreCase("X")) {
                ...
                try { conn.ExecutarComando(adSql); } catch (Exception e) {}
            }
        }
    }
