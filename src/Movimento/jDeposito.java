...
public class jDeposito extends javax.swing.JInternalFrame {
    ...
    private void FillBancos() {
        ...
        ResultSet brs = null;
        try {
            brs = conn.AbrirTabela(sql, ResultSet.CONCUR_READ_ONLY);
    ...
    private void jbtDepositarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbtDepositarActionPerformed
        ...
        if (LerValor.StringToFloat(jVRDN.getText()) > 0) {
            ...
            String dn_auxSql = FuncoesGlobais.Subst(auxSql, new String[] {"DEP","DN",sVar,
                               ...
                               sAut});
            conn.ExecutarComando(dn_auxSql);
        }
        ...
        if (LerValor.StringToFloat(jVRCH.getText()) > 0) {
            ...
            String ch_auxSql = FuncoesGlobais.Subst(auxSql, new String[] {"DEP","CH",sVar,
                               Dates.DateFormata("yyyy/MM/dd", new Date()),Dates.DateFormata("yyyy/MM/dd", new Date()),
                               sAut});
            conn.ExecutarComando(ch_auxSql);
        }
        ...
        if (LerValor.StringToFloat(jVRCP.getText()) > 0) {
            ...
            String cp_auxSql = FuncoesGlobais.Subst(auxSql, new String[] {"DEP","CP",sVar,
                               Dates.DateFormata("yyyy/MM/dd", new Date()),Dates.DateFormata("yyyy/MM/dd", new Date()),
                               sAut});
            conn.ExecutarComando(cp_auxSql);
        }
    ...
    public float Imprimir() throws SQLException {
        ...
        if (VariaveisGlobais.Deposito.split(",")[1].equalsIgnoreCase("TXT")) {
            ImprimeDepositos(nAut, new String[][] {{jVRCH.getText(),jNCH.getText()}, {jVRCP.getText(),jNCP.getText()}, {jVRDN.getText()}, {jTotDepositar.getText()}}, "F");
        } else {
   ...
   public void ImprimeDepositosPDF(float nAut, String[][] ValorRec, String cutPaper) {
        ...
        pdf.close();
        //pdf.print();
        new toPrint(pathName + docName, VariaveisGlobais.Deposito.split(",")[0],VariaveisGlobais.Deposito.split(",")[1],VariaveisGlobais.Deposito.split(",")[2]);
        pdf.setPathName("");
        pdf.setDocName("");
   }
    
    public void ImprimeDepositos(float nAut, String[][] ValorRec, String cutPaper) {
      int i = 0;
      Collections gVar = VariaveisGlobais.dCliente;
      ...
      // hResult = FuncoesGlobais.OpenTransicao("RC") 
        ...
        String docID = ""; //.ps";
        String docName = "reports/Recibos/" + "DS_" + Dates.DateFormata("dd-MM-yyyy", new Date()) + "_" + FuncoesGlobais.StrZero(String.valueOf((int)nAut), 7) + docID;        
        //String pathName = "reports/Recibos/" + Dates.iYear(new Date()) + "/" + Dates.Month(new Date()) + "/";
        ...
        printer.Print(JavaPOS.ESCLAMATION(70) + JavaPOS.ESC_A(1) + JavaPOS.REVERSO + "  D E P O S I T O S   " + JavaPOS.NORMAL, 1);
        ...
        for (int c=0; c< jDep.getRowCount(); c++) {
            //if ("true".equals(jDep.getModel().getValueAt(c, 7).toString().toLowerCase())) {
            if ((Boolean)jDep.getModel().getValueAt(c, 7)) {
                printer.Print(JavaPOS.ESCLAMATION(65) + jDep.getModel().getValueAt(c, 0).toString() + " ", 0);
                printer.Print(JavaPOS.ESCLAMATION(65) + jDep.getModel().getValueAt(c, 1).toString() + " ", 0);
                printer.Print(JavaPOS.ESCLAMATION(65) + new Pad(jDep.getModel().getValueAt(c, 2).toString(),3).CPad() + " ", 0);
                printer.Print(JavaPOS.ESCLAMATION(65) + new Pad(jDep.getModel().getValueAt(c, 3).toString(),5).CPad() + " ", 0);
                printer.Print(JavaPOS.ESCLAMATION(65) + new Pad(jDep.getModel().getValueAt(c, 4).toString(),10).CPad() + " ", 0);
                printer.Print(JavaPOS.ESCLAMATION(65) + new Pad(jDep.getModel().getValueAt(c, 5).toString(),10).LPad() + " ", 0);
                printer.Print(JavaPOS.ESCLAMATION(65) + jDep.getModel().getValueAt(c, 6).toString(), 1);
            }
        }

        printer.Print("",1);
        printer.Print(JavaPOS.ESCLAMATION(65) + "Banco Depositado: ", 0);
        printer.Print(JavaPOS.ESCLAMATION(65) + jBancosCod.getSelectedItem().toString() + " ", 0);
        printer.Print(JavaPOS.ESCLAMATION(65) + jBancosNome.getSelectedItem().toString(), 1);
        printer.Print("--------------------------------------------------------",1);

        printer.Print(JavaPOS.ESCLAMATION(65) + JavaPOS.ESC_A(2) + "Total de Cheques Comuns.......... ",0);
        printer.Print(JavaPOS.ESCLAMATION(65) + "(" + new Pad(ValorRec[0][1],3).CPad() + ") ",0);
        printer.Print(JavaPOS.ESCLAMATION(65) + new Pad(ValorRec[0][0],15).LPad(),1);
        printer.Print(JavaPOS.ESCLAMATION(65) + JavaPOS.ESC_A(2) + "Total de Cheques Pre-Datados..... ",0);
        printer.Print(JavaPOS.ESCLAMATION(65) + "(" + new Pad(ValorRec[1][1],3).CPad() + ") ",0);
        printer.Print(JavaPOS.ESCLAMATION(65) + new Pad(ValorRec[1][0],15).LPad(),1);
        printer.Print(JavaPOS.ESCLAMATION(65) + JavaPOS.ESC_A(2) + "Total de Dinheiro....................... ",0);
        printer.Print(JavaPOS.ESCLAMATION(65) + new Pad(ValorRec[2][0],15).LPad(),1);
        printer.Print(JavaPOS.ESCLAMATION(65) + JavaPOS.ESC_A(2) + "Total Geral a Depositar................. ",0);
        printer.Print(JavaPOS.ESCLAMATION(65) + new Pad(ValorRec[3][0],15).LPad(),1);
        ...
        if (nAut > 0) {
            // Imprimir Autenticação
            printer.Print(JavaPOS.ESC_A(1) + JavaPOS.ESCLAMATION(65) + JavaPOS.REVERSO + VariaveisGlobais.dCliente.get("marca").trim() + JavaPOS.NORMAL + "DS" + FuncoesGlobais.StrZero(String.valueOf((int)nAut), 7) + "-1" + Dates.DateFormata("ddMMyyyyHHmmss", new Date()) + FuncoesGlobais.GravaValores(ValorRec[3][0], 2) + VariaveisGlobais.usuario,1);
            // Codigo de barra
        ...
        //printer.Close();
        new toPrint(docName, VariaveisGlobais.Deposito.split(",")[0],VariaveisGlobais.Deposito.split(",")[1],VariaveisGlobais.Deposito.split(",")[2]);
        printer.setDocName("");
    }   
