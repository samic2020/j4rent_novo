 ...
 * @author supervisor
 */
public class jRazaoAcumulado_old extends javax.swing.JInternalFrame {
    DbMain conn = VariaveisGlobais.conexao;
    String where = "";
    
    /** Creates new form jRazao */
    public jRazaoAcumulado_old() {
        initComponents();
    }
...
//                jbarra2.setValue((100 * posit++)/ fim);
    ...
    private float[] fAVI_SOC_OLD() {
        //int pos = -1; pos = where.indexOf("dtrecebimento >=");
        //if (pos > -1) where = where.substring(0,pos) + where.substring(pos + 34);
        String sql = "SELECT campo FROM auxiliar WHERE Upper(conta) = 'AVI' " + where + " and (CAST(contrato as unsigned integer) >= 01 AND CAST(contrato as unsigned integer) <= 10) ORDER BY rc_aut";
        ResultSet rs = conn.AbrirTabela(sql, ResultSet.CONCUR_READ_ONLY);
        int fim = DbMain.RecordCount(rs); int posit = 1;
...
//                try {Thread.sleep(1000);} catch (InterruptedException e) {}
    ...
    private float[] fAVI_SOC() {
        String sql = "SELECT SUM(if(RetAvTipoRid2(campo)='CRE',RetAvValorRid2(campo),0)) cre, SUM(if(RetAvTipoRid2(campo)='DEB',RetAvValorRid2(campo),0)) deb FROM jgeral.avisos where rid = 3 and tag != 'X';";
        ResultSet rs = conn.AbrirTabela(sql, ResultSet.CONCUR_READ_ONLY);
        int fim = DbMain.RecordCount(rs); int posit = 1;
    ...
    private String BuscaNomeCampo(String campo) {
        ...
        if (LerValor.isNumeric(campo)) {
            try {
                ...
                Logger.getLogger(jRazaoAcumulado_old.class.getName()).log(Level.SEVERE, null, ex);
            }
        } else {
