...
//       jNomeLoca.setRenderer(new MyRenderer());
    ...
    private boolean BuscaNNumero(String nnumero) {
        ...
        String[][] boleta = null; boolean ret = false;
        try {
            if (nnumero.contains("PIX")) {
                boleta = conn.LerCamposTabela(new String[] {"contrato"}, "RECIBO", "nnumero = '" + nnumero + "'");
            } else boleta = conn.LerCamposTabela(new String[] {"contrato"}, "RECIBO", "nnumero REGEXP '...." + nnumero + "$'"); //"nnumero Like '%" + nnumero + "%'");
        ...
        if (boleta == null) {
            ...
            for (i = 0; i < jContrato.getItemCount(); i++) {
                if (jContrato.getItemAt(i).toString().trim().equalsIgnoreCase(boleta[0][3])) {
                    achei = true;
                    break;
                }
            }
...
//        tRec.vrAREC = LerValor.StringToFloat(jResto.getText());
    ...
    private void initComponents() {
        ...
        jLabel10.setBackground(new java.awt.Color(191, 191, 191));
            ...
            .addGroup(layout.createSequentialGroup()
                ...
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addGroup(layout.createSequentialGroup()
                        ...
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addComponent(jpRecebe, javax.swing.GroupLayout.PREFERRED_SIZE, 475, javax.swing.GroupLayout.PREFERRED_SIZE)
    ...
    private void FillNNumero() {
        ...
        if (!sSql.isEmpty()) {
            ...
            try {
                while (imResult.next()) {
                    String tNNumero = imResult.getString("NNUMERO");
                    String pNNumero = tNNumero.contains("PIX") ? tNNumero : String.valueOf(Double.valueOf(tNNumero.substring(3))).replace(".0", "");
                    //jNNumero.addItem(String.valueOf(Double.valueOf(imResult.getString("NNUMERO").substring(3))).replace(".0", ""));
                    jNNumero.addItem(pNNumero);
                }
            } catch (SQLException ex) {
                ex.printStackTrace();
            }
...
//        if (VariaveisGlobais.DefaultPrinterMode.equalsIgnoreCase("NORMAL")) {
        if (VariaveisGlobais.Recibo.split(",")[1].equalsIgnoreCase("TXT")) {
            ImprimeRecibo(-1, new String[][] {},jVrRecibo.getText(), "F",1,1f);
        } else {
            //ImprimeReciboThermica(-1, new String[][] {},jVrRecibo.getText(), "F",1,1f);
...
//                if (VariaveisGlobais.DefaultPrinterMode.equalsIgnoreCase("NORMAL")) {            
                if (VariaveisGlobais.Recibo.split(",")[1].equalsIgnoreCase("TXT")) {            
                    ImprimeRecibo(nAut, aTrancicao, LerValor.floatToCurrency(rc, 2), "F",1,vias);
                } else {
...
//                    if (VariaveisGlobais.DefaultPrinterMode.equalsIgnoreCase("NORMAL")) {
                    if (VariaveisGlobais.Recibo.split(",")[1].equalsIgnoreCase("TXT")) {
                        ImprimeRecibo(nAut, lcto,LerValor.floatToCurrency(rc, 2), (i==vias ? "F" : "P"),i+1,vias);
                    } else {
...
//                        if (VariaveisGlobais.DefaultPrinterMode.equalsIgnoreCase("NORMAL")) {
                    ...
                    while (c<aTrancicao.length) {
                        ...
                        if (LerValor.StringToFloat(LerValor.floatToCurrency(rc,2)) <= LerValor.StringToFloat(LerValor.floatToCurrency(dn + ch,2))) {
                            ...
                            for (int v=1;v<=nvias;v++) {
                                if (VariaveisGlobais.Recibo.split(",")[1].equalsIgnoreCase("TXT")) {
                                    ImprimeRecibo(nAut, especies, LerValor.floatToCurrency(rc, 2), (i==vias ? "F" : "P"),i+1,vias);
                                } else {
    ...
    public void ImprimeRecibo(float nAut, String[][] Valores, String ValorRec, String cutPaper, int via, float nRecibos) {
       System.out.println("ImprimeRecibo");
      int i = 0;
      ResultSet hResult;
      Collections gVar = VariaveisGlobais.dCliente;
      ...
      // hResult = FuncoesGlobais.OpenTransicao("RC")
        ...
        String docName = "reports/Recibos/" + "RC_" + Dates.DateFormata("ddMMyyyy", new Date()) + "_" + FuncoesGlobais.StrZero(String.valueOf((int)nAut), 7) + "-" + 
        ...
        if (gVar.get("cnpj") != null) {
            if (gVar.get("tipodoc") != null) {
        ...
        printer.Print(JavaPOS.ESCLAMATION(70) + JavaPOS.ESC_A(1) + (nAut >0 ? gVar.get("recibo") : "D E M O N S T R A T I V O") + JavaPOS.NORMAL, 1);
        ...
        if (nAut > 0) {
            ...
            printer.Print(JavaPOS.ESCLAMATION(65) + JavaPOS.ESC_A(1) + "Este recibo nao quita debitos anteriores.", 1);
            printer.Print(JavaPOS.ESCLAMATION(65) + JavaPOS.GS_EXC(0) + JavaPOS.ESC_A(0) + "",1);

            // Imprimir Autenticação
            printer.Print(JavaPOS.ESCLAMATION(65) + JavaPOS.ESC_A(1) + JavaPOS.ESCLAMATION(65) + JavaPOS.REVERSO + VariaveisGlobais.dCliente.get("marca").trim() + 
                    JavaPOS.NORMAL + "RC" + FuncoesGlobais.StrZero(String.valueOf((int)nAut), 7) + "-" +
                    FuncoesGlobais.StrZero(String.valueOf((int)via), 2) + "/" + FuncoesGlobais.StrZero(String.valueOf((int)nRecibos), 1) + 
                    Dates.DateFormata("ddMMyyyyHHmmss", new Date()) + " " + JavaPOS.ESC_E(1) + 
                    FuncoesGlobais.GravaValores(ValorRec, 2) + JavaPOS.NORMAL + " " + VariaveisGlobais.usuario,1);
            // Codigo de barra
    ...
    private void ListaCamposRecibo(PrinterPOS printer) {
        ...
        for (i=0; i <= maxcpos - 1; i++) {
            ...
            printer.Print(JavaPOS.ESCLAMATION(65) + JavaPOS.ESC_A(0) + saida, newLine);
        }
    }

   public void ImprimeReciboThermica(float nAut, String[][] Valores, String ValorRec, String cutPaper,int via, float nRecibos) {
       System.out.println("ImprimeReciboThermica");
       ...
       Object[][] linhas = {};
        ...
        linhas = ListaCamposRecibo_new(linhas, Printer);
        ...
        tmp = new Object[][] {
                            ...
                            {jVrRecibo.getText(),null,2,new Font("Times New Roman",Font.BOLD,9), Color.BLACK, Color.WHITE}
                         };
        linhas =  Printer.adcLine(linhas, tmp);
...
//
            ...
            // Imprimir Autenticação
            tmp = new Object[][] {
                                    {VariaveisGlobais.dCliente.get("marca").trim() + "RC" + FuncoesGlobais.StrZero(String.valueOf((int)nAut).replace(".0", ""), 7) + "-" +
                                    FuncoesGlobais.StrZero(String.valueOf((int)via), 2) + "/" + FuncoesGlobais.StrZero(String.valueOf((int)nRecibos), 2) + 
                                    Dates.DateFormata("ddMMyyyyHHmmss", new Date()) + FuncoesGlobais.GravaValores(ValorRec, 2) + VariaveisGlobais.usuario,null,0,new java.awt.Font("Arial",java.awt.Font.BOLD,7), Color.BLACK, Color.WHITE}
                                };
            linhas =  Printer.adcLine(linhas, tmp);
        }
        ...
        for (int k=1;k<=6;k++) { 
            ...
            String docID = "_" + contrato; // + ".ps";
            String docName = "reports/Recibos/" + "RC_" + Dates.DateFormata("ddMMyyyy", new Date()) + "_" + 
...
//            np.toPrint();
   ...
   public void ImprimeReciboPDF(float nAut, String[][] Valores, String ValorRec, String cutPaper,int via, float nRecibos) {
    ...
    private Object[][] ListaCamposRecibo_pdf() {
        ...
        for (i=0; i <= maxcpos - 1; i++) {
            if (jctCampos.getComponent(i) instanceof JLabel) {
                ...
                if (VariaveisGlobais.bShowCotaParcela) {
                    if (nmCampo.toLowerCase().trim().indexOf("cota") > -1) {
                        pcota = new Pad(("00/00".equals(saida.trim()) || "00/0000".equals(saida.trim()) ? "       " : saida), 7).LPad();
                        newLine = 0;
                        posAnt = "2";
                    } else {
                        if ("2".equals(posAnt)) {
    ...
    private Object[][] ListaCamposRecibo_new(Object[][] linhas, jThermica Printer) {
        String cCampos[] = VariaveisGlobais.ccampos.split(";");
        Arrays.sort (cCampos, new Comparator()
        {
            private int pos1 = 3;
