 ...
 * jExtorno.java
 *
 * Created on 13/06/2011, 17:17:28
 */
package Movimento;
...
public class jExtorno extends javax.swing.JInternalFrame {
    ...
    private boolean ExtornaRecibos(String nrAut) {
        ...
        try {
            ...
            // Extorna Critica
            String selectSQL = "SELECT distinct contrato, dtvencimento FROM recibo WHERE AUTENTICACAO = '" + nrAut + "';";
            ResultSet hrs = conn.AbrirTabela(selectSQL, ResultSet.CONCUR_READ_ONLY);
            try {
                while (hrs.next()) {
                    new CriticaExtrato(-1).DelCritica(hrs.getString("contrato"), hrs.getString("dtvencimento"));                    
                }
    ...
    private boolean ExtornaExtratos(String nrAut) {
        ...
        try {
            ...
            String mRGPRP = "", sdant = "";
            try {
                mRGPRP = conn.LerCamposTabela(new String[] {"cx_rgprp"}, "caixa", FuncoesGlobais.Subst("cx_doc = 'ET' AND cx_aut = '&1.'", new String[] {nrAut}))[0][3];
                String[][] vrsdant = conn.LerCamposTabela(new String[] {"pr_sdant"}, "extrato", FuncoesGlobais.Subst("et_aut = '&1.'", new String[] {nrAut}));
                if (vrsdant != null) { sdant = vrsdant[0][3]; } else { sdant = null; }
